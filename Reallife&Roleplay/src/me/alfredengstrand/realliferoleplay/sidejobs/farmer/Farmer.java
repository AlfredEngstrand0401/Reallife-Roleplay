package me.alfredengstrand.realliferoleplay.sidejobs.farmer;
...
public class Farmer implements Job, Listener {
	...
	public Farmer(UUID uuid) {
		...
		Bukkit.getPlayer(uuid).getInventory().addItem(new ItemStack(Material.GOLDEN_HOE));
	}
	...
	public String getTitle() {
		return WorkTitles.FARMER;
	}

	@Override
	public void setupBossbar() {
		if (this.getPlayer() != null && this.getPlayer().isOnline()) {
			setBossbar(Bukkit.createBossBar("Farming Level | " + level, BarColor.GREEN, BarStyle.SOLID));
	...
	public void update() {
		if (xp >= 1.0) {
			this.xp = 0;
			this.level += 1;
			this.bossbar.setTitle("Farming Level | " + level);
	...
	public void complete() {
		...
		UserManager.getUser(uuid).addJobXP(XPLog.GET_FARMING_XP, xp);
